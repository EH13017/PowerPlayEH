...
@TeleOp(name = "TeleOp FieldCentric", group = "Testing")
...
//      manageLEDColors();
   ...
   private void ProMotorControl(double left_stick_y, double left_stick_x, double right_stick_x) {
      ...
      double globalAngleRad = Math.toRadians(_EHGyro.GetHeadingEH());

      double r = Math.hypot(powerLeftX, powerLeftY);
      double robotAngle = Math.atan2(powerLeftY, powerLeftX) - (Math.PI/4) - globalAngleRad;
